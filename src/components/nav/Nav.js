...
function Nav() {
  const [btnShow, setBtnOnShow] = useState(false);
  const [disableWindow, setOnwidow] = useState(false);
  const [logined, setLogin] = useState("로그인");
  const [logouted, setLogout] = useState("로그아웃"),
    handleOnShow = () => {
      setBtnOnShow(!btnShow);
      //, () => console.log(this.state.btnShow);
    };

  const handleOnWindow = () => {
    setOnwidow(!disableWindow);
  };
        ...
        <ActiveWindow>
          ...
          <SearchBox>
            <form>
              ...
              />
              <input
                onClick={handleOnWindow}
                onBlur={handleOnWindow}
                type="serch"
          ...
          <InformationBtn>
            ...
            <span></span>
            <Link
              to={logouted ? "/main" : "/singinentry"}
              onClick={kakaoLogout}
            >
              {localStorage.getItem("token") ? logouted : "로그인"}
            </Link>
            <span></span>
            <Link
              to="#"
              onFocus={() => setBtnOnShow(!btnShow)}
              onBlur={handleOnShow}
            >
              고객센터
              <img
