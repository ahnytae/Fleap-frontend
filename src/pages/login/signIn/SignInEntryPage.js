...
import KakaoLogin from "../signUp/KakaoLogin";
  ...
  loginKakao = () => {
    Kakao.Auth.login({
      success: (authObj) => {
        ...
        fetch(`http://192.168.0.7:8000/user/kakao`, {
          method: "POST",
          headers: {
    ...
    return (
      <MainInner>
        <MainWrapper>
          ...
          <Footer>
            <Link to="/signupentrypage">회원가입</Link>
          </Footer>
        </MainWrapper>
        {/* <button onClick={this.kakaoLogout}> 로그아웃</button> */}
      </MainInner>
    );
  }
}
